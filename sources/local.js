...
class LocalDirectorySource {
    ...
    readZipMetadata(zip) {
        try {
            ...
            const metaEntry = zipEntries.find(e => /(^|\/)metadata\.json$/i.test(e.entryName)); // PATCH15: auch in Unterordnern suchen
        ...
        for (const base of this.rootPaths) {
            ...
            for (const sub of subdirs) {
                ...
                for (const ent of entries) {
                    ...
                    try {
                        const _st = await fs.stat(zipFull);
                        const _ce = _zipScanCache[zipFull];
                        let has, zipMeta;
                        if (_ce && _ce.m === _st.mtimeMs && _ce.s === _st.size) {
                            // PATCH8: ZIP scan disk cache — cache hit, skip AdmZip read
                            has = _ce.h;
                            zipMeta = _ce.z;
                        } else {
                            const zip = new AdmZip(zipFull);
                            const zipEntries = zip.getEntries();
                            has = { poster: false, background: false, thumbnail: false,
                                     clearlogo: false, trailer: false, theme: false, motion: false };
                            for (const ext of exts) {
                                const rePoster = new RegExp(`(^|/)poster\\.${ext}$`, 'i');
                                const reBg = new RegExp(`(^|/)background\\.${ext}$`, 'i');
                                const reThumb = new RegExp(`(^|/)(thumb|thumbnail)\\.${ext}$`, 'i');
                                ...
                                if (zipEntries.some(e => reBg.test(e.entryName))) has.background = true;
                                if (zipEntries.some(e => reThumb.test(e.entryName))) has.thumbnail = true;
                                if (zipEntries.some(e => reClearLogo.test(e.entryName))) has.clearlogo = true;
                            }
                            const videoExts = ['mp4', 'mkv', 'avi', 'mov', 'webm', 'm4v'];
                            for (const ext of videoExts) {
                                if (zipEntries.some(e => new RegExp(`(^|/)trailer\\.${ext}$`, 'i').test(e.entryName))) { has.trailer = true; break; }
                            }
                            for (const ext of videoExts) {
                                if (zipEntries.some(e => new RegExp(`(^|/)motion\\.${ext}$`, 'i').test(e.entryName))) { has.motion = true; break; }
                            }
                            const audioExts = ['mp3', 'flac', 'wav', 'ogg', 'm4a', 'aac'];
                            for (const ext of audioExts) {
                                if (zipEntries.some(e => new RegExp(`(^|/)theme\\.${ext}$`, 'i').test(e.entryName))) { has.theme = true; break; }
                            }
                        ...
                        let found = false;
                        if (want === 'motion') {
                            found = Boolean(has.poster && has.motion && this.isMotionZipMetadata(zipMeta));
                        } else { found = has[want]; }
                        if (!found) continue;
                        results.push({ name: ent.name, path: zipFull, size: st.size, modified: st.mtime,
                            extension: 'zip',
                            directory: want === 'background' ? 'backgrounds' : want === 'motion' ? 'motion' : 'posters',
                            type: want, zipHas: has, zipMetadata: zipMeta });
                        seen.add(baseName);
                    } catch (e) {
    ...
    createMediaItem(file, metadata) {
        ...
        return {
            ...
            related: enrichedMeta.related || null,
            source: 'local',
            tmdbId: enrichedMeta.tmdbId || null,
            imdbId: enrichedMeta.imdbId || null,
