...
**Stand:** 2026-08-30 (basierend auf Version 3.0.1z-18)
...
| 53 | 3-Schichten-Backup-Strategie | utils/configBackup.js (FILE_WHITELIST erweitert um playlists/filmliste/profiles), scripts/backup/backup-to-nas.sh, scripts/backup/systemd/*, docs/BACKUP-STRATEGY.md | Lokaler Config-Backup + NAS-SMB-Mirror + NAS-Snapshots. Täglich 03:30 Config-Backup, 04:00 NAS-Mirror. Stumm-Skip bei NAS-Offline, `--delete`-Mirror (Versionierung via NAS-Snapshots). |
| 54 | Cloudflare-Tunnel (Pi-Setup-Doku, kein Code im Repo) | docs/CLOUDFLARE-TUNNEL.md, docs/INDEX.md | `cloudflared` als Public-Endpoint für Posterrama via `https://posterrama.example.com/`. Cloudflare Access (E-Mail-OTP) vorgeschaltet, kein Port-Forwarding nötig, TLS terminiert auf Cloudflare. Reicht WebSocket+SSE durch. Voraussetzung: `app.set('trust proxy', 1)` in server.js (war bereits gesetzt). |
| 55 | Monitor-Power-Watcher Self-Heal (Doku-Update + Pi-Skript-Edit) | docs/MONITOR-POWER-WATCHER.md (Self-Heal-Sektion + Linger-Setup-Schritt) | Behebt Boot-Race: Watcher startet vor Chromium, findet beim Init nichts zu stoppen, reagiert dann nie wieder (Übergang prev=off→curr=off triggert klassisch nichts). Self-Heal-Block in der Loop schickt SIGSTOP idempotent nach, wenn `curr=off` und Chromium-PIDs nicht im T-Status. Pi-Skript `~/.local/bin/monitor-power-watch.sh` (nicht im Repo) wurde entsprechend ergänzt. |
| 56 | Bluetooth-Audio Auto-Reconnect (Pi-Setup-Doku, kein Code im Repo) | docs/BLUETOOTH-AUDIO.md, docs/INDEX.md | Robuster Reconnect des Cinema-Lautsprechers (Soundcore 3) nach Power-Cycle, Standby, Reichweiten-Verlust. Drei Schichten: bluez `Trusted=yes` + main.conf-Tuning (`AutoEnable`, `FastConnectable`, `ReconnectAttempts`), User-systemd-Watcher mit 30s-Polling, PipeWire-Stack mit automatischem Default-Sink. `loginctl enable-linger` als Boot-Voraussetzung. |
| 57 | Emby-Sync UI/Backend Bugfixes (4 Bugs) | routes/emby-sync.js, public/admin.js, lib/emby-sync.js, poster-updater/download-trailers.py, poster-updater/download-trailers-youtube.py | (1) POST `/ignored` ersetzte statt anzuhängen + schrieb Class-Backing-Fields (`_xxx`/`env`/`defaults`) in config.json — Fix: `config.config` statt Class-Instance serialisieren, Self-Heal-Strip beim Save. (2) `initEmbySync` aus IIFE-Closure für Cross-IIFE-Aufrufer via `window.initEmbySync` exportiert. (3) Auto-Playlist-Race-Condition: Post-Job-Refresh pollt `runner.isPosterRunning()` und ruft `updateAutoPlaylist` nach Download-Ende mit frischem ZIP-Set erneut auf. (4) Trailer-Scripts versteh `[tmdb:NNN]`-Hint-Format aus Patch 51 (Hint vor Format-Match abstrippen). |
| 58 | Download-Buttons für Logs/Reports/PosterPacks-Liste | lib/poster-updater-runner.js (In-Memory-Log-Buffer + getPosterLog/getTrailerLog), routes/poster-updater.js (3 Endpoints: /run/log, /trailers/run/log, /posterpacks/list), routes/emby-sync.js (/last-report/download), public/admin.html (4 Download-Buttons in PosterPack/Trailer/Emby-Sync-Cards) | 4 neue Download-Buttons im Admin-UI: PosterPack-Log + PosterPacks-Liste (PosterPack Card), Trailer-Log (Trailer Card), Sync-Report-JSON (Emby-Sync Card). Logs als 5-MB-Ringbuffer in-memory pro Job. PosterPacks-Liste sortiert über alle 5 Export-Quellen. |
| 59 | NAS-Backup-Performance: Zwei rsync-Calls statt einem | scripts/backup/backup-to-nas.sh, docs/BACKUP-STRATEGY.md | Call 1 (alles außer `media/`): `--modify-window=2` für CIFS-mtime-Toleranz. Call 2 (nur `media/`): zusätzlich `--size-only` weil mp4/zip immutable sind und ihre Bytes-Größe eindeutig ist. Vorher wurde wegen SMB-mtime-Rundungsfehlern die Mehrzahl der Files immer wieder neu übertragen — gemessen 2h 5min → 12:35 min Folge-Lauf, weiter sinkend auf <5 min bei 0-Diff. Pi-Setup-Doku + Repo-Template, lebt als systemd-Service `posterrama-nas-backup.service`. |
| 60 | DELETE-Filmliste räumt in ALLEN 5 Export-Quellen auf | routes/poster-updater.js | `DELETE /api/poster-updater/films/:name` löschte vorher das ZIP nur in `tmdb-export/`; Filme aus `manual/`, `plex-export/`, `jellyfin-emby-export/`, `romm-export/` ließen Karteileichen zurück. Fix: Iteration über alle 5 Quellen via neuer Konstante `ZIP_SOURCES`, plus zusätzliche Sidecar-Suffixe (`.poster.json`). Trailer-Cleanup, trailer-info.json-Cleanup und Playlist-Cleanup waren bereits korrekt. |
| 61 | Auto-Playlist sortiert nach ZIP-Erstellungsdatum statt Emby-DateCreated | lib/emby-sync.js (updateAutoPlaylist), lib/poster-updater-runner.js (neue getAllZipMtimes()) | Bisher sortierte die Auto-Playlist „Die letzten N hinzugefügten Filme" nach Emby's `dateCreated` — d. h. wann der Film auf den Emby-Server kam (oft Jahre her). Frische lokale PosterPack-Erstellungen (etwa nach Delete+Re-Add) erschienen NICHT als „neu" in der Playlist. Fix: Sortierung nach ZIP-mtime; neue runner-Funktion `getAllZipMtimes()` liefert Map<canonicalKey, mtimeMs>, bei mehreren Quellen gewinnt jüngste mtime. Post-Job-Refresh-Helper aus Patch 57 wurde mit umgestellt. |
| 62 | TMDB-Downloader: Bild-/Sprachfilter-Fix + crash-sicherer Cleanup | poster-updater/tmdb-get-posters-direct.py, .gitignore | `language=de-DE` filterte `/images` und `/videos` serverseitig auf Deutsch (kaum Backdrops, keine EN-Poster/Clearlogos/Trailer); EN-Tagline-Fallback wurde vom `language`-Override erschlagen. Fix: `include_image_language=de,en,null`, `include_video_language=de,en`, `setdefault('language')`. Dazu per-Film try/finally-Cleanup + SIGTERM-Handler + Startup-Sweep gegen verwaiste `tmp_`-Ordner, Partial-ZIP-Löschung, HTTP-Status-Check bei Bilddownloads, Null-sichere Felder (runtime/vote_average/iso_639_1), DE-Priorität für Poster/Trailer/Clearlogo. Details: CHANGELOG 3.0.1z-8. |
| 63 | Cinema-Freeze-Fix: YouTube-Fehlerpfad startet Rotation neu | public/cinema/cinema-display.js | Anzeige fror dauerhaft auf einem Plakat ein, wenn der YouTube-Trailer im Embed nicht abspielbar war (z. B. Alterssperre → YT-Fehler 150, Fall „Ich fühl mich Disco"): startTrailerPlayback() stoppte die Rotation, onError/catch starteten sie nie wieder. Fix: onError + catch schalten nach 2 s weiter (showNextPoster + startRotation, analog Lokal-Pfad), plus 45s-Start-Watchdog gegen Age-Gate-Embeds ganz ohne Events (Muster vom Screensaver). Details: CHANGELOG 3.0.1z-9. |
| 64 | Session-Poller-Haertung: Backoff statt Stopp + Multi-Server-Jellyfin | services/jellyfinSessionsPoller.js, services/plexSessionsPoller.js, __tests__/services/jellyfinSessionsPoller.test.js, __tests__/services/plexSessionsPoller.test.js | Beide Poller gaben nach 5 Fehlversuchen dauerhaft auf („too many errors, stopping") — ein temporär offliner Media-Server schaltete die Now-Playing-Erkennung bis zum Server-Neustart ab. Jellyfin pollte zudem nur den ERSTEN enabled Server (find statt filter). Fix: Jellyfin pollt alle enabled Server (Sessions gemerged), Fehler-Isolation pro Server mit Grace-Period, exponentieller Backoff (60s→max 5min) mit Auto-Recovery; Plex symmetrisch. Details: CHANGELOG 3.0.1z-10. |
