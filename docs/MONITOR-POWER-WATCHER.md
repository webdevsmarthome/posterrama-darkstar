...
**Was gemessen wurde (während eines Freezes, 2026-08-30 10:42):** Chromium `S`/`R` mit Renderer-CPU, `grim`-Frames wechseln, Framebuffer-ID der Primary-Plane wechselt (727↔721), Pixelvalve-Vblank-IRQ ~48/s, DDC/CI antwortet, Server gesund. Die zweite Plane an `pixelvalve-2` ist die **Cursor-Plane** (64×64, in der Ecke geparkt) — kein Overlay; Chromium wird regulär in die Primary-Plane komponiert. Pi-seitig gibt es also **kein messbares Signal** für den Freeze. Ein **echter Moduswechsel** (1280x720 → 1920x1080) holt das Bild zurück; ein `--transform`-Zyklus reicht nicht (reine Software-Rotation).

**Ursache — nicht bewiesen, Verdacht HDMI-Link/Monitor:** Drei Symptome treten am selben Gerät auf: (1) das Bild friert ein, obwohl der Pi weiter Frames ausgibt; (2) die DDC/CI-Firmware des Monitors verstummt für 8–13 h; (3) die Kernel-EDID (`/sys/class/drm/card1-HDMI-A-1/edid`) ist dauerhaft **0 Bytes**, während `ddcutil` die EDID per Raw-I2C nur mit Retries liest. DDC/EDID laufen über dieselben Leitungen wie das Bild. Ein marginales HDMI-Kabel/Stecker oder eine instabile Monitor-Firmware erklärt alle drei. Erste Maßnahme daher: **Kabel tauschen (kurz, zertifiziert), Stecker beidseitig neu setzen**; danach prüfen, ob die Kernel-EDID wieder 128/256 Bytes hat. Das im Session-Log gefundene `[backend/drm/libliftoff.c] Atomic commit failed: Device or resource busy` ist ein Einzelereignis ohne zeitlichen Bezug zu den Freezes.
...
| Beweissammler + Sicherheitsnetz | `scanout-watchdog.service` (User-Service, `~/.local/bin/scanout-watchdog.sh`) | Alle 10 s: Frame-Hash (`grim -s 0.25`), Framebuffer-IDs aller Planes an `pixelvalve-2`, HVS-/Pixelvalve-IRQ-Deltas → Rolling-Log `~/.local/state/scanout-watchdog/samples.log` (letzte 24 h). Beim nächsten Freeze zeigt das Log, was der Pi in dem Moment tat. Zusätzlich die konservative Heilregel: Frame in 5 min ≥ 3× geändert, aber keine Plane je eine andere ID → Resync (Cooldown 15 min) — sie greift nur, falls die Flips wirklich stehen bleiben; den Freeze vom 30.08. (Flips liefen) hätte sie **nicht** erkannt. Optional: `PREVENTIVE_RESYNC_MIN=30` in der Service-Umgebung erzwingt alle 30 min einen Resync als Notbehelf (kurzer Flacker) |
| Compositor | `WLR_DRM_NO_ATOMIC=1` in `~/.config/labwc/environment` | Legacy-KMS statt Atomic + libliftoff (auf diesem Pi zweimal als Bugquelle aufgefallen, u. a. `wlr-randr --off`). Prophylaktisch, **Wirkung auf den Freeze unbewiesen**. Greift beim nächsten Start von labwc; ob es aktiv ist, zeigt nur ein Debug-Log (`labwc -d`), nicht `/proc/<pid>/environ` (labwc setzt die Datei-Variablen erst nach dem Start per `setenv`) |
