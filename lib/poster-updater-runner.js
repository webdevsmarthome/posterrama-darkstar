...
const { parseTmdbHint } = require('./zip-tmdb-index');
...
const ALL_EXPORT_DIRS = [
    ...
    // Altordner -- der ZIP-Scan (sources/local.js) liest ihn weiterhin mit
    path.join(PROJECT_ROOT, 'media', 'complete', 'jellyfin-export'),
...
// Ergebniszeile des Trailer-Scripts auswerten. download-trailers.py druckt am Ende
//   TRAILER-SUMMARY downloaded=9 skipped=1262 no_trailer=42 failed=9 total=1323
// dazu je fehlgeschlagenem Video eine Zeile "yt-dlp Fehler: ERROR: [youtube] <id>: ...".
// Hintergrund: Ab Anfang August 2026 scheiterte jeder Download an einem fuenf
// Monate alten yt-dlp ("This video is not available"). Das Script faengt jeden
// Fehler ab und endet mit 0 -- der Runner meldete wochenlang nur "finished,
// code=0", der Fehlertext lag ausschliesslich im Admin-Ringpuffer.
function parseTrailerSummary(log) {
    // "searched=N" (seit z-17: davon per YouTube-Suche geladen) ist optional.
    const m =
        /TRAILER-SUMMARY downloaded=(\d+) skipped=(\d+) no_trailer=(\d+) failed=(\d+) total=(\d+)(?: searched=(\d+))?/.exec(
            log || ''
        );
    if (!m) return null;
    const reasons = [];
    for (const line of log.match(/yt-dlp Fehler: .+/g) || []) {
        // Video-ID entfernen, damit gleiche Ursachen zusammenfallen
        const reason = line
            .replace(/^yt-dlp Fehler: /, '')
            .replace(/\[youtube\] [\w-]+: /, '')
            .trim();
    ...
    return {
        downloaded: Number(m[1]),
        skipped: Number(m[2]),
        noTrailer: Number(m[3]),
        failed: Number(m[4]),
        total: Number(m[5]),
        searched: Number(m[6] || 0),
        reasons,
    };
}

function setOnTrailerJobDone(cb) {
    onTrailerJobDone = typeof cb === 'function' ? cb : null;
}
...
// --- Filmliste CRUD ---
async function readFilmList(filePath = FILMLISTE_PATH) {
    try {
...
async function writeFilmList(films, filePath = FILMLISTE_PATH) {
    // Wortgleiche Doppelzeilen entfernen -- es waren 10 entstanden, die das
    // Trailer-Script faelschlich als Titel-Dubletten (Remakes) wertete.
    const sorted = [...new Set(films)].sort((a, b) => a.localeCompare(b, 'de'));
    // Atomar: Python-Jobs und Admin-UI lesen die Datei jederzeit.
    const tmp = `${filePath}.${process.pid}.tmp`;
    await fsp.writeFile(tmp, sorted.join('\n') + '\n', 'utf8');
 ...
 *    seltener Edge-Case).
 *  - Gleiche [tmdb:N]-ID unter ANDEREM Titel → duplicate (z-20). Die zwei
 *    Emby-Server benennen denselben Film teils unterschiedlich; ohne diesen
 *    Abgleich entstanden 43 Doppel-Paare ("Top Gun - Maverick" / "Top Gun:
 *    Maverick") mit doppeltem PosterPack und Trailer. Auch ein Upgrade wird
 *    blockiert, wenn die ID schon an einer anderen Zeile haengt.
 *
 * Atomar hinter writeLock. filePath ist fuer Tests injizierbar.
 */
async function appendFilms(newFilms, { filePath = FILMLISTE_PATH } = {}) {
    return withFileLock(async () => {
        const existing = await readFilmList(filePath);
        // Map von Title-Year-Key (lowercase NFC) → Index in existing
        const keyToIndex = new Map();
        // Map von TMDB-ID → Index der Zeile, die sie traegt
        const idToIndex = new Map();
        existing.forEach((e, idx) => {
            keyToIndex.set(stripTmdbHint(e).normalize('NFC').toLowerCase(), idx);
            const { tmdbId } = parseTmdbHint(e);
            if (tmdbId && !idToIndex.has(tmdbId)) idToIndex.set(tmdbId, idx);
        });
        ...
        for (const raw of newFilms) {
            ...
            const { tmdbId } = parseTmdbHint(nfc);
            const idIdx = tmdbId ? idToIndex.get(tmdbId) : undefined;

            const existingIdx = keyToIndex.get(baseKey);
            if (existingIdx === undefined) {
                if (idIdx !== undefined) {
                    duplicates.push(nfc);
                    duplicateIds.push({ entry: nfc, existing: resultList[idIdx] });
                    continue;
                }
                resultList.push(nfc);
                keyToIndex.set(baseKey, resultList.length - 1);
                if (tmdbId) idToIndex.set(tmdbId, resultList.length - 1);
                added.push(nfc);
                continue;
            }
            ...
            if (!existingHasHint && newHasHint) {
                if (idIdx !== undefined && idIdx !== existingIdx) {
                    duplicates.push(nfc);
                    duplicateIds.push({ entry: nfc, existing: resultList[idIdx] });
                    continue;
                }
                ...
                if (tmdbId) idToIndex.set(tmdbId, existingIdx);
                upgraded.push(nfc);
            } else {
                duplicates.push(nfc);
            }
        ...
        const removedExactDuplicates = resultList.length - new Set(resultList).size;
        if (added.length > 0 || upgraded.length > 0 || removedExactDuplicates > 0) {
            await writeFilmList(resultList, filePath);
        }
        return { added, upgraded, duplicates, duplicateIds, removedExactDuplicates };
    });
}
...
/**
 * Wie getAllExistingZips, aber zusätzlich mit ZIP-mtime (ms seit Epoch).
 * Genutzt von updateAutoPlaylist für die Sortierung „neueste PosterPacks zuerst".
 * Liegt ein Film-Key in mehreren Quellen, gewinnt die jüngste mtime.
 */
async function getAllZipMtimes(dirs = ALL_EXPORT_DIRS) {
    const map = new Map();
    for (const dir of dirs) {
        try {
            ...
            for (const e of entries) {
                ...
                const key = e.replace(/\.zip$/i, '').normalize('NFC');
                try {
                    const st = await fsp.stat(path.join(dir, e));
                    const existing = map.get(key);
                    if (!existing || st.mtimeMs > existing) {
                        map.set(key, st.mtimeMs);
                    }
                } catch {
                    /* file vanished between readdir and stat — egal */
                }
            }
        } catch (err) {
            if (err.code !== 'ENOENT') throw err;
...
function spawnTrailerJob() {
    ...
    try {
        ...
        // Suchentscheidungen des YouTube-Fallbacks (Zeilen mit 🔎) zusaetzlich ins
        // Server-Log: Das Script schreibt sie mit end='' und haengt das Ergebnis
        // (✅/❌) spaeter an dieselbe Zeile -- deshalb erst bei vollstaendiger Zeile
        // loggen. Wenige Zeilen pro Lauf; der Ringpuffer bleibt die Vollansicht.
        // Nur echte Entscheidungen tragen den [i/n]-Index -- die Zaehlzeile
        // "🔎 davon per Suche: N" der Zusammenfassung nicht.
        const isSearchDecision = line => line.includes('🔎') && /\[\d+\/\d+\]/.test(line);
        let pendingLine = '';
        const logSearchDecisions = text => {
            pendingLine += text;
            const lines = pendingLine.split('\n');
            pendingLine = lines.pop();
            for (const line of lines) {
                if (isSearchDecision(line)) logger.info('poster-updater: ' + line.trim());
            }
        };
        proc.stdout.on('data', chunk => {
            const text = chunk.toString('utf8');
            appendTrailerLog(text);
            logSearchDecisions(text);
        ...
        proc.on('close', code => {
            if (isSearchDecision(pendingLine)) logger.info('poster-updater: ' + pendingLine.trim());
            appendTrailerLog(`\n=== Job beendet ${new Date().toISOString()} (Exit-Code ${code}) ===\n`);
            logger.info('poster-updater: Trailer download finished, code=' + code);
            // Ergebnis ins Server-Log -- als Warnung, sobald etwas fehlschlug.
            const summary = parseTrailerSummary(trailerLog);
            if (!summary) {
                logger.warn(
                    'poster-updater: Trailer-Lauf ohne Ergebniszeile beendet -- Script vorzeitig abgebrochen?',
                    {
                        code,
                        tail: trailerLog.trim().split('\n').slice(-3).join(' | ').slice(0, 300),
                    }
                );
            } else {
                const viaSearch =
                    summary.searched > 0 ? ` (davon ${summary.searched} per YouTube-Suche)` : '';
                const text =
                    `poster-updater: Trailer-Lauf: ${summary.downloaded} geladen${viaSearch}, ` +
                    `${summary.failed} fehlgeschlagen, ${summary.noTrailer} ohne TMDB-Trailer, ` +
                    `${summary.skipped} uebersprungen (${summary.total} Filme)`;
                if (summary.failed > 0 || code !== 0) {
                    logger.warn(text, { code, reasons: summary.reasons });
                } else {
                    logger.info(text, { code });
                }
            }
            if (onTrailerJobDone) {
                try {
                    onTrailerJobDone({ code, summary });
                } catch (err) {
                    logger.warn(
                        'poster-updater: onTrailerJobDone-Hook fehlgeschlagen: ' + err.message
                    );
                }
            }
