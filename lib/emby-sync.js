...
async function updateAutoPlaylist(
    ...
    // Kandidaten: Filme mit vorhandenem ZIP, nach DateCreated desc
    const candidates = allMovies
        .filter(m => existingZipKeys.has(m.canonicalKey) && m.dateCreated)
...
async function runSyncCycle({ logger, config, wsHub, trigger = 'manual' } = {}) {
    ...
    try {
        ...
        // Existierende ZIPs aufbauen (alle 5 Unterordner)
        const existingZipKeys = await runner.getAllExistingZips();
        report.existingZips = existingZipKeys.size;
        ...
        if (toDownload.length > 0) {
            if (embySyncCfg.downloads?.posterPack !== false) {
                try {
                    await runner.appendFilms(toDownload.map(m => m.canonicalKey));
