    ...
    const fetchFromLocal = async () => {
        ...
        try {
            ...
            const normalizeLocalItem = item => {
                ...
                let director = item?.director || item?.metadata?.director || null;
                let directors = item?.directors || item?.metadata?.directors || [];
                let studio = item?.studio || item?.metadata?.studio || null;
                let studios = item?.studios || item?.metadata?.studios || [];
                let resolution = item?.resolution || item?.metadata?.resolution || null;
                let audioCodec = item?.audioCodec || item?.metadata?.audioCodec || null;
                let aspectRatio = item?.aspectRatio || item?.metadata?.aspectRatio || null;
                let hdr = item?.hdr || item?.metadata?.hdr || null;
                let genres = item?.genres || item?.metadata?.genres || item?.genre || item?.metadata?.genre || [];
                ...
                if (isZip) {
                    ...
                    try {
                        if (!zipMetaCache.has(lp)) {
                            ...
                            if (item?.zipHas && typeof item.zipHas === 'object') {
                                ...
                                zipMetaCache.set(lp, {
                                    ...
                                    genres: j.genres || j.genre || null,
                                    director: j.director ?? null,
                                    directors: j.directors || null,
                                    studio: j.studio ?? null,
                                    studios: j.studios || null,
                                    resolution: j.resolution ?? null,
                                    audioCodec: j.audioCodec ?? null,
                        ...
                        if (zm) {
                            ...
                            if (!director && (zm.director || zm.directors?.[0]))
                                director = zm.director || zm.directors[0];
                            if ((!directors || !directors.length) && zm.directors)
                                directors = zm.directors;
                            if (!studio && (zm.studio || zm.studios?.[0]))
                                studio = zm.studio || zm.studios[0];
                            if ((!studios || !studios.length) && zm.studios)
                                studios = zm.studios;
                            if (!resolution && zm.resolution)
                                resolution = zm.resolution;
                            if (!audioCodec && zm.audioCodec)
                                audioCodec = zm.audioCodec;
                            if (!aspectRatio && zm.aspectRatio)
                                aspectRatio = zm.aspectRatio;
                            if (!hdr && zm.hdr) hdr = zm.hdr;
                            if ((!genres || !genres.length) && zm.genres)
                                genres = zm.genres;
                ...
                return {
                    ...
                    directors: directors && directors.length ? directors : [],
                    studio: studio || null,
                    studios: studios && studios.length ? studios : [],
            ...
            for (const it of normalizedAll) {
                ...
                if (!existing) {
                    ...
                    if (!existing.trailerUrl && it.trailerUrl) existing.trailerUrl = it.trailerUrl;
            ...
            let normalized = Array.from(dedupMap.values());

            if (config.localDirectory.requireTrailer === true) {
                // Motion posters and standalone backgrounds never carry a trailerUrl by design
                const isExempt = it =>
                    it.isMotionPoster === true ||
                    String(it.type || '').toLowerCase() === 'motion' ||
                    String(it.directory || '').toLowerCase() === 'backgrounds';
                const before = normalized.length;
                normalized = normalized.filter(
                    it =>
                        isExempt(it) ||
                        (typeof it.trailerUrl === 'string' && it.trailerUrl.length > 0)
                );
                if (before !== normalized.length) {
                    logger.info(
                        `[Local Directory] requireTrailer active: filtered ${before - normalized.length} of ${before} items without trailer`
                    );
                }
            }
            ...
            if (normalized.length > 0) {
                ...
                // NB: kein allMedia.concat hier -- der Aufrufer haengt localResult.value.media
                // an; das zusaetzliche interne Concat hatte jedes lokale Item verdoppelt.
            } else {
