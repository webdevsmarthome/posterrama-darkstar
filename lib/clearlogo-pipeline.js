...
// Cache-Invalidierung
// ---------------------------------------------------------------------
async function invalidateZipScanCache() {
    try {
        if (!fs.existsSync(ZIP_SCAN_CACHE)) return;
        // Cache komplett zuruecksetzen — der naechste Playlist-Refresh
        // baut ihn aus den jetzt aktualisierten ZIPs neu auf.
        await fsp.writeFile(ZIP_SCAN_CACHE, '{}', 'utf8');
        appendLog('--- zip-scan-cache geleert (wird beim naechsten Scan neu aufgebaut) ---\n');
    } catch (err) {
        appendLog(`--- zip-scan-cache-Reset fehlgeschlagen: ${err.message} ---\n`);
    }
}
